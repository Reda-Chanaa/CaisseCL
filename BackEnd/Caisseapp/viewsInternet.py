...
def Internet_files_AMEX_debit(df1, df2):
    ...
    merged = pd.merge(df_cepac, df_seaware, how="outer", on='Transaction')
    merged['Montant init.'] = merged['Montant init.'].replace(np.nan, 0)
    merged['Montant du paiement'] = merged['Montant du paiement'].replace(np.nan, 0)
...
def Internet_cc_phrase(df1, df2, df3):
    ...
    if (df_seaware['Date transaction'].dtypes == 'datetime64[ns]'):
        ...
        df_seaware['date'] = df_seaware['Date transaction'].astype(
            'datetime64[ns]')
    ...
    df_cepac_remise['Montant du paiement'] = df_cepac_remise[
        'Montant du paiement'].astype(float)
    ...
    if (df_cepac_remise['Date du paiement'].dtypes == 'datetime64[ns]'):
        ...
        df_cepac_remise['date'] = pd.to_datetime(
            df_cepac_remise['Date du paiement'], format='%d/%m/%Y %H:%M:%S')
    ...
    df_cepac_attente['Transaction'] = df_cepac_attente['Transaction'].astype(
        str)
    df_cepac_attente['Montant du paiement'] = df_cepac_attente[
        'Montant du paiement'].astype(float)
    ...
    if (df_cepac_attente['Date du paiement'].dtypes == 'datetime64[ns]'):
        ...
        df_cepac_attente['date'] = pd.to_datetime(
            df_cepac_attente['Date du paiement'], format='%d/%m/%Y %H:%M:%S')
    ...
    attenteRemise = df_cepac_remise[df_cepac_remise['date'].astype(
        str).str.contains(str(stri), na=False)]
    print(len(attenteRemise))

    attenteRemise_1 = df_cepac_remise[~df_cepac_remise['Transaction'].isin(
        attenteRemise["Transaction"].values)]
    attenteRemise_1.reset_index(drop=True, inplace=True)

    datetime_object = datetime.strptime(str(stri), '%Y-%m-%d')
    print(datetime_object)

    attenteRemiseJour = df_cepac_attente[(df_cepac_attente["date"] >= datetime(
        datetime_object.year, datetime_object.month, datetime_object.day, 22,
        30, 00)) & (df_cepac_attente["date"] <= datetime(
